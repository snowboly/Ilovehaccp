                  ...
                  <>
                    ...
                    <div id="audit-report" className="bg-white border border-slate-200 p-8 rounded-3xl shadow-sm space-y-6">
                        <div className="flex items-center justify-between border-b border-slate-100 pb-6">
                            <div>
                                <h2 className="text-2xl font-black text-slate-900">HACCP Draft Check Summary</h2>
                                <p className="text-sm text-slate-500">Generated by iLoveHACCP · Assisted Draft</p>
                            </div>
                            <div className="flex items-center gap-4">
                                <Link href={editUrl} className="bg-white text-slate-700 border border-slate-200 px-3 py-2 rounded-lg text-xs font-bold hover:bg-slate-50 transition-colors">
                                    Edit draft answers
                                </Link>
                            </div>
                        </div>
                        ...
                        <div className="space-y-6">
                            <section className="space-y-3">
                                ...
                                <div className="grid md:grid-cols-2 gap-4">
                                    <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                                        <p className="text-xs font-bold uppercase text-slate-400">Declared sections</p>
                                        <p className="text-base font-semibold text-slate-900">Required HACCP sections declared</p>
                                    </div>
                                    <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                                        <p className="text-xs font-bold uppercase text-slate-400">Draft state</p>
                                        <p className="text-base font-semibold text-slate-900">Automated checks recorded</p>
                                    </div>
                                </div>
                            </section>
                            ...
                            <section className="space-y-3">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h3 className="font-bold text-slate-900">Automated checks performed</h3>
                                </div>
                                <p className="text-xs text-slate-500">Automated checks reference the declared information in each required section.</p>
                                <ul className="space-y-2">
                                    {validationReport?.section_2_strengths?.map((s: string, i: number) => (
                            ...
                            <section className="space-y-3">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h3 className="font-bold text-slate-900">Sections requiring attention</h3>
                                </div>
                                <p className="text-xs text-slate-500">
                                    Items below require clarification based on the declared information.
                                </p>
                                <ul className="list-none space-y-3">
                                    {validationReport?.section_3_weaknesses_risks?.map((w: any, i: number) => {
                                        const step = sectionToStep(w.section || '');
                                        return (
                                            <li key={i} className="flex items-start gap-3 rounded-xl border border-slate-100 bg-slate-50 p-3 text-slate-600">
                                                <AlertTriangle className="w-4 h-4 text-amber-500 shrink-0 mt-1" />
                                                <div className="flex-1 space-y-1">
                                                    <p className="text-sm text-slate-700">Declared: {w.weakness}</p>
                                                    <p className="text-xs text-slate-500">Section: {w.section || 'Unspecified section'}</p>
                                                    <p className="text-xs text-slate-500">Action: Edit this section to clarify the declared information.</p>
                                                </div>
                                                <Link
                                                    href={editUrlForStep(step)}
                                                    className="flex items-center gap-1 text-slate-600 hover:text-slate-900 text-xs font-bold whitespace-nowrap shrink-0"
                                                >
                                ...
                                {isPaid && validationReport?.advisory_recommendations && validationReport.advisory_recommendations.length > 0 && (
                                    <div className={`p${'t-4'} border-t border-slate-100`}>
                                        ...
                                        <p className="text-xs text-slate-500 mb-3">
                                            Additional context tied to declared gaps and required clarifications.
                                        </p>
                                                ...
                                                return (
                                                    <div key={i} className="bg-white border border-slate-200 rounded-2xl p-4">
                                                        <div className="flex items-start justify-between gap-4 mb-2">
                                                            <h5 className="font-bold text-slate-800 text-xs uppercase tracking-wide">{rec.issue_summary}</h5>
                                                            <Link href={editUrlForStep(recStep)} className="flex items-center gap-1 text-slate-600 hover:text-slate-900 text-xs font-bold whitespace-nowrap">
                                                                <Pencil className="w-3 h-3" />
                                                                Edit
                                                            </Link>
                                                        </div>
                                                        <p className="text-sm text-slate-600 leading-relaxed">{rec.expert_commentary}</p>
                                                        <div className="text-xs text-slate-400 mt-3">
                                                            Related: {rec.related_builder_section} · HACCP Principle {rec.related_haccp_principle}
                                                        </div>
                                                    </div>
                                                );
                                            })}

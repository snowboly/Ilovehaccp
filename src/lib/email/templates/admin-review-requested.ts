type AdminReviewRequestedEmailProps = {
  appUrl: string;
  planId: string;
};

export function adminReviewRequestedEmailHtml(props: AdminReviewRequestedEmailProps): string {
  const adminUrl = `${props.appUrl}/admin/plans/${props.planId}/review`;
...
<body>
  <div class="wrapper">
    <div class="container">
      ...
      <div class="content">
        <h1 class="h1">Review requested</h1>

        <p class="text">A plan has requested a review.</p>
        <p class="text">Open the admin review page to proceed.</p>

        <div class="button-container">
          <a href="${adminUrl}" class="button" target="_blank">Open Admin Review</a>
        </div>

        <p class="text" style="font-size:14px; color:#94a3b8; margin-top:32px; text-align:center;">
          This is an internal notification for the admin inbox.
        </p>
      </div>

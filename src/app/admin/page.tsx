...
export default async function AdminPage() {
  ...
  const [
    ...
    supabaseService.from('plans').select('*', { count: 'exact', head: true }).eq('payment_status', 'paid').eq('tier', 'professional'),
    supabaseService.from('plans').select('*', { count: 'exact', head: true }).eq('payment_status', 'paid').eq('tier', 'expert'),
            ...
            recentLogs.map((log: any) => (
              <div key={log.id} className="p-4 flex items-center justify-between text-sm hover:bg-slate-50 transition-colors">
                <div className="flex items-center gap-4">
                  ...
                  <span className="font-bold text-slate-700 uppercase tracking-wider text-xs">{log.action}</span>
                </div>
                <span className="text-slate-500 text-xs font-mono max-w-xs truncate">{JSON.stringify(log.details)}</span>
              </div>
            ))
          ) : (
